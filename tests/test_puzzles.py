...
from src.puzzles import PUZZLES, get_puzzle, solution_lines_of
from src.rules import XiangqiGame, legal_moves
...
def test_multi_answer_either_solves():
    ...
    pid = "pawn_side_d8"
    assert client.post(f"/api/puzzles/{pid}/load", headers=h).status_code == 200
    r = client.post(f"/api/puzzles/{pid}/check", json={"uci": "e8f8"}, headers=h)
...
def test_solution_lines_auto_replies_and_continues():
    ...
    assert client.post(f"/api/puzzles/{pid}/load", headers=h).status_code == 200
    r1 = client.post(f"/api/puzzles/{pid}/check", json={"uci": "a0a9"}, headers=h)
    ...
    r2 = client.post(f"/api/puzzles/{pid}/check", json={"uci": "a9a8"}, headers=h)
...
def test_wrong_move_does_not_solve_or_puzzle():
    ...
    pid = "pawn_side_d8"
    assert client.post(f"/api/puzzles/{pid}/load", headers=h).status_code == 200
    r = client.post(f"/api/puzzles/{pid}/check", json={"uci": "e8e7"}, headers=h)
    assert r.status_code == 200
    body = r.json()
...
def test_session_isolation():
    ...
    client.post("/api/game/new", json={}, headers=ha)
    client.post("/api/game/new", json={}, headers=hb)
    client.post("/api/game/move", json={"uci": "b0c2"}, headers=ha)
    sa = client.get("/api/game/state", headers=ha).json()
    sb = client.get("/api/game/state", headers=hb).json()
...
def test_room_invite_join_and_full():
    create = client.post("/api/rooms", json={"name": "红方", "color": "red"})
    assert create.status_code == 200
    room = create.json()["room_id"]
    join = client.post(f"/api/rooms/{room}/join", json={"name": "黑方"})
    assert join.status_code == 200
    assert join.json()["color"] == "black"
    assert join.json().get("token")
    third = client.post(f"/api/rooms/{room}/join", json={"name": "旁观"})
...
def test_challenge_load_sets_puzzle_check_path():
    ...
    pid = "mate_rook_a0d0"
    r = client.post(f"/api/challenges/{pid}/load", headers=h)
    assert r.status_code == 200
    body = r.json()
    assert body.get("challenge", {}).get("id") == pid
    # 非正解不应通关
    bad = client.post(f"/api/puzzles/{pid}/check", json={"uci": "a0a1"}, headers=h)
    assert bad.status_code == 200
    assert bad.json()["correct"] is False
    ok = client.post(f"/api/puzzles/{pid}/check", json={"uci": "a0d0"}, headers=h)
...
def test_bad_uci_and_fen_are_400():
    ...
    assert client.post("/api/game/move", json={"uci": "z9z9"}, headers=h).status_code == 400
    assert client.post("/api/game/targets", json={"square": "oops"}, headers=h).status_code == 400
    bad_fen = "xnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1"
    assert client.post("/api/game/load-fen", json={"fen": bad_fen}, headers=h).status_code == 400
